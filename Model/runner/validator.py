...
class ValidatorLSTM:
    def __init__(self,
        ...
        self.device = device
        self.logger = logger
        self.angle_bin_vector = self.caculate_angle_bin_vector(bin_num=10)
        a = 1

    def caculate_angle_bin_vector(self, bin_num=10):
    ...
    def pred_to_angle(self, pred, epsilon=1e-8, bin_num=10):
...
class ValidatorDHM:
    def __init__(self,
        ...
        self.device = device
        self.logger = logger
        self.angle_bin_vector = self.caculate_angle_bin_vector(bin_num=12)
        a = 1
    ...
    def val(self, val_loader):
        ...
        with torch.no_grad():
            for index, (x, y, true_label) in enumerate(val_loader):
                ...
                az_true = true_label[:, 0].cpu().numpy()
                zen_true = true_label[:, 1].cpu().numpy()
                az_pred = regression_result[:, 0].cpu().numpy()
                zen_pred = regression_result[:, 1].cpu().numpy()
                # az_pred, zen_pred = self.pred_to_angle(outputs.cpu().numpy())
                sz1 = np.sin(zen_true)
                cz1 = np.cos(zen_true)
                sz2 = np.sin(zen_pred)
